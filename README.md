...
|------|------|---------|-------------|
| `initialScene` | `SceneConfig` | *required* | The first scene to render |
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `source` | `{ uri: string }` | *required* | URL to a `.glb` or `.gltf` model |
...
| `scaleSensitivity` | `number` | `0.3` | Dampens pinch speed (0–1) |
...
function MyScene() {
  ...
  return (
    <>
      ...
      <Button title="Next" onPress={() => navigator.push({ scene: NextScene })} />
    </>
  );
}
```
